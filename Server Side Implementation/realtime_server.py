...
def extract_features(url):
    ...
    features = {
        ...
        'use_of_ip': int(bool(re.search(
            ...
            r'(?:[a-fA-F0-9]{1,4}:){7}[a-fA-F0-9]{1,4}', parsed_url.hostname))) if parsed_url.hostname else 0,
...
def email_fetchncheck(user, password, email_server, clf, whitelisted_domains, server_email , server_pass):
    try:
...
def monitoring_instance(email, app_pass, mail_server, clf, whitelisted_domains, alert_server_email, alert_server_app_pass):
    while True:
        email_fetchncheck(email, app_pass, mail_server, clf, whitelisted_domains, alert_server_email, alert_server_app_pass)
        time.sleep(10)
...
def server_service():
    ...
    for credential in credential_list:
        ...
        p = multiprocessing.Process(target=monitoring_instance, args=(email, app_pass,mail_server, clf, whitelisted_domains, alert_server_email, alert_server_app_pass))
